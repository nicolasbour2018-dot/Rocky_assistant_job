...
- Toute table est déclarée sur `rocky.system.db.metadata` (dans le fichier d'accès SQL de son module, importé par
  `migrations/env.py`) **et** créée par une migration. `test_declared_tables_match_migrations` échoue sinon.
...
- `events` ne subit jamais `UPDATE`, `DELETE` ni `TRUNCATE` (la base les refuse) : une correction est un nouvel
  événement.
- Type d'événement : `<module>.<fait_au_passé>` (`offres.decision_recorded`).
